...
async function main() {
    ...
    const server = new Server({ name: "magic-codex", version: "0.3.0" }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        if (name === "list") {
            ...
            const filtered = all.filter((rec) => {
                if (parsed.role && rec.role !== parsed.role)
                    return false;
                if (parsed.status && rec.status !== parsed.status)
                    return false;
                if (parsed.issue_id && rec.issue_id !== parsed.issue_id)
                    return false;
                if (parsed.has_pr !== undefined) {
                    const hasPr = rec.pr_number != null;
                    if (hasPr !== parsed.has_pr)
                        return false;
                }
                if (parsed.stale_after_seconds !== undefined && rec.ended_at) {
                    const ageSec = (now - Date.parse(rec.ended_at)) / 1000;
                    if (ageSec < parsed.stale_after_seconds)
                        return false;
                }
                return true;
            });
